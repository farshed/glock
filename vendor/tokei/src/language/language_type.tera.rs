...
impl LanguageType {
    ...
    pub fn from_shebang<P: AsRef<Path>>(entry: P) -> Option<Self> {
        ...
        Self::from_shebang_slice(&buf[..len])
    }

    /// Get language from the shebang line of an in-memory buffer.
    ///
    /// The counterpart to [`LanguageType::from_shebang`] for callers that
    /// already hold the file's contents, or that have no filesystem to read
    /// from at all. Only the first line is examined.
    ///
    /// ```
    /// use tokei::LanguageType;
    ///
    /// let sh = LanguageType::from_shebang_slice(b"#!/bin/bash\necho hi\n");
    ///
    /// assert_eq!(sh, Some(LanguageType::Bash));
    /// ```
    #[must_use]
    pub fn from_shebang_slice(buf: &[u8]) -> Option<Self> {
